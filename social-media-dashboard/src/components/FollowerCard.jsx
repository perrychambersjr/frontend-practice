...
const FollowerCard = ({index, icon, name, followers, title, trend, trendCount, color, theme}) => {
  ...
  return (
    <div className={`${isGradient ? "" : "border-t-4"} rounded-lg overflow-hidden shadow-lg cursor-pointer`}
      ...
      <div className="bg-gray-200 dark:bg-slate-900 h-50 rounded flex flex-col justify-center items-center">
          <div className="flex items-center mb-4 gap-3">
            <img src={icon} alt="icon" className="h-4 w-4"/>
            <p className="font-bold text-l dark:text-white">{name}</p>
          </div>
          <h2 className="font-bold text-5xl mb-2 dark:text-white">{followers}</h2>
